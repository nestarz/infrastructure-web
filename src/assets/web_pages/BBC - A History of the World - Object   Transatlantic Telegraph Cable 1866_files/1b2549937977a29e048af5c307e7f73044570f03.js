...
var blq = new function (){
    ...
    */
    ...
    function initIdentityStatusBar() {
        ...
        function isSignedIn() {
            return document.cookie.match(/IDENTITY=/); 
        }
        
        /**
         * Retrieve the user's name/displayname from their 'IDENTITY' cookie.
         * @return string or null
         */
        function getNameFromCookie() {
            var cookieParts = isSignedIn() ? decodeURIComponent(document.cookie.match(/IDENTITY=([^;]*)/)[1]).split('|') : '';
            if (cookieParts[2]) {
                return decodeURIComponent(cookieParts[2].replace(/\+/g, ' '));
            }
            if (cookieParts[1] && cookieParts[1].charAt(0) === '~') {
                return null;
            }
        ...
        /**
        * @description Returns a substring of text of the specified length followed by an elipsis.
        ...
        */
        ...
        /**
        * @description Returns a substring of text of the specified length followed by an elipsis.
        * @param {DOMElement} elm The element to be updated.
        * @param {string} name The users name.
        */
        function updateSigninText(elm, name) {
            var shortName = shorten(name, 14);
            elm.textContent = elm.innerText = shortName;
            if (shortName !== name) {
                elm.title = name;
            }
        }
         ...
         */
        function updateForAuthorisedState() {
            var ctaElm = el('blq-idstatus-text'),
                ctaLink = el('blq-idstatus-link'),
                signedin = isSignedIn(),
                signinUrl,
                name;
            ...
            if (ctaElm && ctaLink) {
                if (signedin) {
                    if (name = getNameFromCookie()) {
                        updateSigninText(ctaElm, name);
                    }
                    else {
                        require(['id-statusbar-config'], function (config) {
                            name = config.translation_signedin;
                            updateSigninText(ctaElm, name);
                        });
                    }
                }
                    ...
                    require(['id-statusbar-config'], function (config) {
                        name = config.translation_signedout;
                        updateSigninText(ctaElm, name);
                        
                        ctaLink.href = config.signin_url;
                    });
                }
            }
        }
