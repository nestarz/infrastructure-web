...
const {
  splitStringBy, requestGetBody, isTld, isDomain,
} = require('./utils.js');
require('array-flat-polyfill');

// cache
const cacheTldWhoisServer = {
...
const whoisDomain = async (domain, {
  host = null, timeout = 15000, follow = 2, raw = false,
} = {}) => {
  ...
  if (!host) {
    const tld = await whoisTld(domain, { timeout, raw });
  ...
  while (host && follow) {
    ...
    if (nextWhoisServer) {
      ...
      if (nextWhoisServer.includes('://')) {
        const parsedUrl = url.parse(nextWhoisServer);
