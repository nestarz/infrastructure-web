...
describe('Whoiser', () => {
  describe('#whoiser()', () => {
    ...
    it('should return IP WHOIS for "1.1.1.1"', async () => {
      const whois = await whoiser('1.1.1.1');
    ...
    it('should return AS WHOIS for "15169"', async () => {
      const whois = await whoiser('15169');
  ...
  describe('#whoiser.tld()', () => {
    ...
    it('should return WHOIS for ".香港" - IDN', async () => {
      const whois = await whoiser('.香港');
      assert.equal(whois.domain, '香港', 'TLD doesn\'t match');
      assert.equal(whois.whois, 'whois.hkirc.hk', 'WHOIS server doesn\'t match');
    });

    it('should return WHOIS for ".XN--J6W193G" - IDN', async () => {
      const whois = await whoiser('.XN--J6W193G');
      assert.equal(whois.domain, '香港', 'TLD doesn\'t match');
      assert.equal(whois.whois, 'whois.hkirc.hk', 'WHOIS server doesn\'t match');
    });

    it('should reject for invalid TLD format', () => {
      assert.rejects(whoiser.tld('-abc'));
    });

    it('should reject for non-existing TLD', () => {
  ...
  describe('#whoiser.domain()', () => {
    it('should return WHOIS for "blog.google"', async () => {
    ...
    it('should return WHOIS for "cloudflare.com" from "whois.cloudflare.com" server (host option)', async () => {
    ...
    it('should return WHOIS for "laye.red" from top-level whois server (follow option)', async () => {
    ...
    it('should return WHOIS for "google.eu" when whois data is "label:_EOL_value"', async () => {
    ...
    it('should return WHOIS for "mañana.com" - IDN', async () => {
    ...
    it('should return WHOIS for "XN--MAANA-PTA.COM" - IDN', async () => {
