...
const requestGetBody = (url) => new Promise((resolve, reject) => {
  https.get(url, (resp) => {
    let data = '';
...
const isDomain = (domain) => {
  ...
  return labels.length > 1 && labels.every((label, index) => (index ? labelTest.test(label) && !label.startsWith('-') && !label.endsWith('-') : isTld(label)));
};
